...
ext.tabs.onRemoved.addListener((tabId) => {
  ...
  if (globalThis.chrome?.offscreen) stopTabAudio(tabId).catch(() => {});
});
...
ext.runtime.onMessage.addListener((message, sender, sendResponse) => {
  Promise.resolve(handleMessage(message, sender)).then(sendResponse, (error) => sendResponse({ ok: false, error: cleanError(error) }));
  return true;
});
...
async function handleMessage(message, sender) {
  switch (message?.type) {
    ...
    case "CAPTURE_VISIBLE": {
      ...
      const dataUrl = await ext.tabs.captureVisibleTab(windowId, { format: "jpeg", quality: 92 });
      return { dataUrl };
    }
    case "START_TAB_AUDIO":
      return startTabAudio(sender.tab);
    case "STOP_TAB_AUDIO":
      return stopTabAudio(sender.tab?.id);
    case "OFFSCREEN_TRANSCRIPT":
      if (message.tabId != null) await sendToTopFrame(message.tabId, { type: "TRANSCRIPT", text: String(message.text ?? "") });
      return { ok: true };
    case "FOCUS_EDITABLE":
    ...
    case "AUTO_TYPE": {
      ...
      if (!frame && tabId != null && ext.storage.session) frame = (await ext.storage.session.get(`focus:${tabId}`))[`focus:${tabId}`];
    ...
    case "CANCEL_AUTO_TYPE": {
      ...
      if (tabId != null && frame) await ext.tabs.sendMessage(tabId, { type: "TYPE_CANCEL" }, { frameId: frame.frameId }).catch?.(() => {});
      return { ok: true };
    }
    default:
      return undefined;
...
async function startTabAudio(tab) {
  if (!tab?.id || !globalThis.chrome?.tabCapture || !globalThis.chrome?.offscreen) {
    return { ok: false, fallback: "displayMedia" };
  }
  const streamId = await chrome.tabCapture.getMediaStreamId({ targetTabId: tab.id });
  await ensureOffscreenDocument();
  await chrome.runtime.sendMessage({ type: "OFFSCREEN_START_AUDIO", streamId, tabId: tab.id });
  return { ok: true, mode: "tabCapture" };
}

async function stopTabAudio(tabId) {
  if (!globalThis.chrome?.offscreen) return { ok: false };
  if (!(await chrome.offscreen.hasDocument())) return { ok: true };
  await chrome.runtime.sendMessage({ type: "OFFSCREEN_STOP_AUDIO", tabId });
...
async function ensureOffscreenDocument() {
  ...
  await chrome.offscreen.createDocument({
    ...
    reasons: ["USER_MEDIA"],
    justification: "Process tab audio for user-requested live transcription",
  });
}
...
async function responseMessage(response) {
  ...
  return detail || `Provider request failed (${response.status})`;
}
...
async function showActionError(tabId, title) {
  ...
  setTimeout(() => {
    ...
    ext.action.setTitle({ tabId, title: "Toggle Hermes" }).catch?.(() => {});
  }, 3500);
}
